...
const TranscriptMeta = {
    ...
    observingQuery: {
        $or: [
            {reviewCompletedOn: {$exists: false}, outbound: {$exists: false}},
    ...
    fields: fields => {
        fields.pescCollegeTranscript = Document.GeneratedField('self', ['pescCollegeTranscriptXML'], Transcript.Meta.observingQuery, function (fields) {
            ...
            if (!fields.pescCollegeTranscriptXML) {
                console.log ("in pescCollegeTranscript generatedField for document _id:" + fields._id + ", pescCollegeTranscriptXML is undefined");
                return [fields._id, undefined];
            } else {
                const object = xml2js.parseStringSync(fields.pescCollegeTranscriptXML, {
                    attrkey: '@',
                    xmlns: false,
                    ignoreAttrs: true,
                    explicitArray: false,
                ...
                console.log ("in pescCollegeTranscript generatedField for document _id:" + fields._id + ", CollegeTranscript.TransmissionData.DocumentID" + object ? object.CollegeTranscript.TransmissionData.DocumentID : "not parsed");
